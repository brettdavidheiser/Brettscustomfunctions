...
clumpedbyCyc <- function (rawdata){
  ...
  #rawdata <- did_files %>% iso_get_standards_info(with_ratios = TRUE) %>% mutate(delta_value = case_when(ratio_name == "R 17O/16O" ~ 5, ratio_name == "R 15N/14N" ~ 42, TRUE ~ delta_value)) #this is another way to do it.
  
  #Brett used the wrong method. I used PBC and so have to subset data
  raw_data_w_measurement_info <- subset(raw_data_w_measurement_info, file_id != "12427__all$dIB2_1000_0.did")
